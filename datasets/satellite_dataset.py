...
def generate_rays_from_metadata(d, img_downscale):
...
def init_scaling_params(json_dir, img_downscale):
    if not os.path.exists(f"{json_dir}/scene.loc"):
        ...
        for json_p in all_json:
            ...
            rays = generate_rays_from_metadata(d, img_downscale)
            all_rays += [rays]
        ...
        sat_utils.write_dict_to_json(d, f"{json_dir}/scene.loc")
        print("... done !")
    else:
        print("Scaling params exist in", f"{json_dir}/scene.loc")


def generate_train_cache(json_dir, cache_dir, img_downscale):
    os.makedirs(cache_dir, exist_ok=True)

    with open(os.path.join(json_dir, "train.txt"), "r") as f:
        json_files = f.read().split("\n")
    json_files = [os.path.join(json_dir, json_p) for json_p in json_files]
    ...
    for json_p in json_files:
        ...
        cache_path = "{}/{}.data".format(cache_dir, img_id)
        print("Generating cache", cache_path)
        if not os.path.exists(cache_path):
            rays = generate_rays_from_metadata(metadata, img_downscale)
...
class TrainSatelliteDataset(Dataset):

    def __init__(self, root_dir, img_dir, img_downscale=1.0, cache_dir=None):
        """
        ...
        assert os.path.exists(f"{self.json_dir}/scene.loc"), f"{self.json_dir}/scene.loc does not exist"
        ...
        self.all_rays, self.all_rgbs, self.all_ids = self.load_data(self.json_files)
...
class ValidationSatelliteDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_id = sat_utils.get_file_id(metadata["img"])
        h, w = int(metadata["height"] // self.img_downscale), int(metadata["width"] // self.img_downscale)
